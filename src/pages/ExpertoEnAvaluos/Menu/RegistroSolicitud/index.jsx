...
import Anexo3 from "../../../../components/Organismo/Prueba/Anexo3";
import DataTableFormSolicitud from "../../../../components/Molecula/DataTableFormSolicitud";
import Anexo4 from "../../../../components/Organismo/Anexo4/Anexo4";
...
// import {
//   deleteRegistroLocalizacionSlice,
//   fetchDescripcionRegistroLocalizacionSlice,
//   postRegistroLocalizacionSlice,
//   putRegistroLocalizacionSlice,
...
export default function RegistroSolicitud() {
  ...
  const [localizacionReal, uselocalizacionReal] = useState({
    lat: -12.04224,
    lng: -77.0670592,
  });
      ...
      if (data.id_solicitud) {
        dispatch(
          putRegistroSolicitudSlice({
            ...
            perito_asignado: service.postMultiSelect(
              data.perito_asignado,
              "name"
            ),
          })
        );
  ...
  const FormikValidacionPerito = useFormik({
    initialValues: servicios.emptyValidacionPerito || {
      responsable: "",
      nombre: "",
      email: "",
      direccion: "",
    ...
    onSubmit: (data) => {
      FormikValidacionPerito.resetForm();
    },
    enableReinitialize: true,
  });
  const isFormFieldValid2 = (name) =>
    !!(
      FormikValidacionPerito.touched[name] &&
      FormikValidacionPerito.errors[name]
    );
  const getFormErrorMessage2 = (name) => {
    return (
      isFormFieldValid2(name) && (
        <small className="p-error">{FormikValidacionPerito.errors[name]}</small>
      )
    );
  };
  ...
  const FormikLocalizacion = useFormik({
    initialValues: {
      ...
      coordenadasX: localizacionReal.lat,
      coordenadasY: localizacionReal.lng,
    } || {
  ...
  const editProduct = (dataObj) => {
    setData({
      ...
      perito_asignado: service.updateMultiSelect(
        dataObj.perito_asignado,
        "name"
      ),
  ...
  // Editar los datos de creacion de PDF
  const editProductPDF = (dataObj) => {
    setProductDialog2(true);
  };
  // Editar los datos de creacion de localizacion
  const editProductLocalizacion = (dataObj) => {
  ...
  // Cerrar el formulacio anexo4 de pdf
  const hideDialog2 = () => {
    setProductDialog2(false);
  };
  // Cerrar formulacio de ubicacion
  const hideDialog3 = () => {
  ...
  const actionBodyTemplatePDF = (rowData) => {
    return (
      <div className="actions">
        <Button
          icon="pi pi-file-pdf"
          className="p-button-rounded p-button-info mr-2"
          onClick={() => editProductPDF(rowData)}
        />
      </div>
    );
  };
  ...
  const botonesSolicitantes = () => {
    return (
      <div className="containerBtn">
        <Button label="Aceptar" className="p-button-rounded  mr-2" />
        <Button label="Rechazar" className="p-button-rounded  mr-2" />
        <Button label="Devolver" className="p-button-rounded  mr-2" />
      </div>
    );
  };
  return (
      ...
      <div className="col-12">
        <div className="card">
          ...
          <DataTableFormSolicitud
            ...
            actionBodyTemplatePDF={actionBodyTemplatePDF}
          ...
          />
          <Anexo4
            productDialog2={productDialog2}
            hideDialog2={hideDialog2}
            // saveProduct2={saveProduct2}
            etFormErrorMessage={getFormErrorMessage2}
          />
          <Localizacion
