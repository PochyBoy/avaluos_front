...
// Redux
import {
  deleteRegistroTipoDePersonaSlice,
  fetchAllRegistroTipoDePersonaSlice,
  postRegistroTipoDePersonaSlice,
  putRegistroTipoDePersonaSlice,
} from "../../store/slices/RegistroTipoDePersona";
import { useDispatch, useSelector } from "react-redux";

export default function TipoDePersona() {
  let empty = {
    id_tipodepersona: null,
    codigo: "",
    descripcion: "",
    estado: false,
  };
  ...
  //Redux useSelector
  const RegistroTipoDePersona = useSelector(
    (state) => state.RegistroTipoDePersona
  );

  const correlativoTabla = () => {
    let codigocero = "0000000000";
    let contador = 1;
    if (RegistroTipoDePersona.allRegistroTipoDePersona.length > 0)
      contador =
        parseInt(
          RegistroTipoDePersona.allRegistroTipoDePersona[
            RegistroTipoDePersona.allRegistroTipoDePersona.length - 1
          ].codigo
        ) + 1;
  ...
  useEffect(() => {
    dispatch(fetchAllRegistroTipoDePersonaSlice());
  }, [dispatch]);
  ...
  const formik = useFormik({
    ...
    onSubmit: (data) => {
      if (data.id_tipodepersona) {
        dispatch(putRegistroTipoDePersonaSlice(data));
        toast.current.show({
          severity: "success",
          summary: "Successful",
      ...
      dispatch(postRegistroTipoDePersonaSlice(data));
      toast.current.show({
        severity: "success",
        summary: "Successful",
        detail: "Product Updated",
  ...
  // ----------------------------
  const deleteProduct = () => {
    dispatch(deleteRegistroTipoDePersonaSlice(data));
    setDeleteProductDialog(false);
    setData(empty);
    toast.current.show({
      severity: "success",
    ...
    <div className="grid crud-demo">
      <div className="col-12">
        <div className="card">
          ...
          <DataTableForm
            dt={dt}
            products={RegistroTipoDePersona.allRegistroTipoDePersona}
