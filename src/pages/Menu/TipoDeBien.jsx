...
// Redux
import {
  deleteRegistroTipoDeBienSlice,
  fetchAllRegistroTipoDeBienSlice,
  postRegistroTipoDeBienSlice,
  putRegistroTipoDeBienSlice,
} from "../../store/slices/RegistroTipoDeBien";
import { useDispatch, useSelector } from "react-redux";
export default function TipoDeBien() {
  let empty = {
    id_tipodebien: null,
    codigo: "",
    descripcion: "",
  ...
  //Redux useSelector
  const RegistroTipoDeBien = useSelector((state) => state.RegistroTipoDeBien);

  const correlativoTabla = () => {
    let codigocero = "0000000000";
    let contador = 1;
    if (RegistroTipoDeBien.allRegistroTipoDeBien.length > 0)
      contador =
        parseInt(
          RegistroTipoDeBien.allRegistroTipoDeBien[
            RegistroTipoDeBien.allRegistroTipoDeBien.length - 1
          ].codigo
        ) + 1;
  ...
  useEffect(() => {
    dispatch(fetchAllRegistroTipoDeBienSlice());
  }, [dispatch]);
  ...
  const formik = useFormik({
    ...
    onSubmit: (data) => {
      if (data.id_tipodebien) {
        if (data.descripcion.trim()) {
          dispatch(putRegistroTipoDeBienSlice(data));
          toast.current.show({
            severity: "success",
            summary: "Successful",
      ...
      dispatch(postRegistroTipoDeBienSlice(newData));
      setProductDialog(false);
      formik.resetForm();
    },
  ...
  // ----------------------------
  const deleteProduct = () => {
    dispatch(deleteRegistroTipoDeBienSlice(data));
    setDeleteProductDialog(false);
    setData(empty);
    toast.current.show({
      severity: "success",
    ...
    <div className="grid crud-demo">
      <div className="col-12">
        <div className="card">
          ...
          <DataTableForm
            dt={dt}
            products={RegistroTipoDeBien.allRegistroTipoDeBien}
            selectedProducts={selectedProducts}
            setSelectedProducts={setSelectedProducts}
            globalFilter={globalFilter}
            header={HeaderTableDataInputText({
              setGlobalFilter: setGlobalFilter,
              TitleName: "Registro de Tipo de Bien",
            })}
