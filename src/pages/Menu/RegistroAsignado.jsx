...
// Redux
import {
  deleteRegistroAsignadoSlice,
  fetchAllRegistroAsignadoSlice,
  postRegistroAsignadoSlice,
  putRegistroAsignadoSlice,
} from "../../store/slices/RegistroAsignado";
import { useDispatch, useSelector } from "react-redux";
export default function RegistroAsignado() {
  let empty = {
    id_asignacion: null,
    codigo: "",
    descripcion: "",
  ...
  //Redux useSelector
  const RegistroAsignado = useSelector((state) => state.RegistroAsignado);

  const correlativoTabla = () => {
    let codigocero = "0000000000";
    let contador = 1;
    if (RegistroAsignado.allRegistroAsignado.length > 0)
      contador =
        parseInt(
          RegistroAsignado.allRegistroAsignado[
            RegistroAsignado.allRegistroAsignado.length - 1
          ].codigo
        ) + 1;
  ...
  useEffect(() => {
    dispatch(fetchAllRegistroAsignadoSlice());
  }, [dispatch]);
    ...
    onSubmit: (data) => {
      if (data.id_asignacion) {
        if (data.descripcion.trim()) {
          dispatch(putRegistroAsignadoSlice(data));
          toast.current.show({
            severity: "success",
            summary: "Successful",
      ...
      dispatch(postRegistroAsignadoSlice(newData));
      setProductDialog(false);
      formik.resetForm();
    },
  ...
  // ----------------------------
  const deleteProduct = () => {
    dispatch(deleteRegistroAsignadoSlice(data));
    setDeleteProductDialog(false);
    setData(empty);
    toast.current.show({
      severity: "success",
    ...
    <div className="grid crud-demo">
      <div className="col-12">
        <div className="card">
          ...
          <DataTableForm
            dt={dt}
            products={RegistroAsignado.allRegistroAsignado}
            selectedProducts={selectedProducts}
            setSelectedProducts={setSelectedProducts}
            globalFilter={globalFilter}
            header={HeaderTableDataInputText({
              setGlobalFilter: setGlobalFilter,
              TitleName: "Registro de Asignado",
            })}
