...
// Redux
import {
  deleteRegistroBienSlice,
  fetchAllRegistroBienSlice,
  postRegistroBienSlice,
  putRegistroBienSlice,
...
export default function Bien() {
  ...
  const correlativoTabla = () => {
    ...
    let contador = 1;
    if (RegistroBien.allRegistroBien.length > 0)
      contador =
        parseInt(
          RegistroBien.allRegistroBien[RegistroBien.allRegistroBien.length - 1]
            .codigo
        ) + 1;
    ...
    onSubmit: (data) => {
      if (data.id_bien) {
        if (data.descripcion.trim()) {
          dispatch(putRegistroBienSlice(data));
          toast.current.show({
            severity: "success",
            summary: "Successful",
      ...
      dispatch(postRegistroBienSlice(data));
      toast.current.show({
        severity: "success",
        summary: "Successful",
        detail: "Product Updated",
  ...
  // ----------------------------
  const deleteProduct = () => {
    dispatch(deleteRegistroBienSlice(data));
    setDeleteProductDialog(false);
    setData(empty);
    toast.current.show({
      severity: "success",
    ...
    <div className="grid crud-demo">
      <div className="col-12">
        <div className="card">
          ...
          <DataTableForm
            dt={dt}
            products={RegistroBien.allRegistroBien}
            selectedProducts={selectedProducts}
            setSelectedProducts={setSelectedProducts}
            globalFilter={globalFilter}
            header={HeaderTableDataInputText({
              setGlobalFilter: setGlobalFilter,
              TitleName: "Registro de Bien",
            })}
